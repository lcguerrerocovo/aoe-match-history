...
| `liveMatchHandler.ts` | Live match handler: two-phase paginated fetch of observable advertisements (5 fast pages returned immediately, up to 15 pages / 3000 matches fetched in background; pages within each phase fetched concurrently), stale-while-revalidate cache (25s fresh / 60s stale) with request coalescing — in-flight background pages block new fetch cycles so their work isn't discarded; a failed background crawl keeps the cache in `fast` phase (`X-Partial`). Match deduplication, normalizes to typed `LiveMatch[]`. Enriches ratings server-side via `player_latest_rating` table — maps match_type_id → leaderboard_id via `rating_leaderboard_mapping`, then does batch PK lookups per leaderboard. Per-player rating cache (90s TTL) keyed by `profileId:leaderboardId`. |
...
|--------|------|---------|-------|
| GET | `/api/live` | Live matches via `findObservableAdvertisements` (authenticated, two-phase paginated: 5 fast pages returned immediately, up to 15 pages fetched in background, concurrent within each phase). Stale-while-revalidate cache (25s fresh / 60s stale) with request coalescing. Returns `X-Partial: 1` header when background pages are still loading or the crawl failed partway. ELO ratings enriched server-side from `player_latest_rating` table (leaderboard-specific — RM 1v1 matches get RM 1v1 ratings, not TG). Optional `?profile_ids=` filters to matches containing those players (single page, no cache). | 30s |
